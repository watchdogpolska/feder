...
class TaskQuerySet(models.QuerySet):
    ...
    def survey_left(self):
        return self.annotate(
            survey_left=models.F("survey_done") - models.F("survey_required")
        )
    ...
    def survey_stats(self):
        result = self.aggregate(
            done_count=models.Sum("survey_done"),
            required_count=models.Sum("survey_required"),
        )
        if result["required_count"]:
            result["progress"] = (
                result.get("done_count", 0) / result.get("required_count", 0) * 100
            )
        else:
            result["progress"] = 0
        return result
...
class Task(TimeStampedModel):
    ...
    survey_required = models.SmallIntegerField(
        verbose_name=_("Required survey count"), help_text=TASK_REQUIRED_TEXT, default=2
    )
    survey_done = models.SmallIntegerField(
        verbose_name=_("Done survey count"), default=0
    )
    ...
    def get_next_for_user(self, user):
        task = (
            Task.objects.by_monitoring(self.case.monitoring)
            .exclude_by_user(user)
            .first()
        )
        if not task:
    ...
    def verify(self):
        object_list = list(
            Answer.objects.filter(survey__task=self)
        ...
        if object_list[0].survey.credibility > 2:  # selected survey always pass
            return True
        for question, answer_by_question in groupby(
            object_list, lambda x: x.question_id
        ):
...
class Answer(models.Model):
    ...
    def get_answer_text(self):
        return self.question.modulator.get_answer_text(
            self.question.definition, self.content
        )

    def get_answer_columns(self):
        return self.question.modulator.get_answer_columns(
            self.question.definition, self.content
        )
...
def increase_task_survey_done(sender, instance, created, **kwargs):
    if created:
        Task.objects.filter(pk=instance.task_id).update(
            survey_done=models.F("survey_done") + 1
        )
...
def raise_alert_if_mismatch(sender, instance, created, **kwargs):
    if created and not instance.task.verify():
        ...
        Alert.objects.create(
            monitoring=instance.task.case.monitoring,
            reason=reason_text,
