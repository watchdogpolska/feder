...
THANK_TEXT = _(
    "Thank you for your submission. It is approaching us to know the "
    + "truth, by obtaining reliable data."
)

EXHAUSTED_TEXT = _(
    "Thank you for your help. Unfortunately, all the tasks "
...
class TaskDetailView(SelectRelatedMixin, PrefetchRelatedMixin, DetailView):
    ...
    def get_user_survey(self):
        try:
            return (
                self.object.survey_set.with_full_answer()
                .of_user(self.request.user, self.request.light_user)
                .get()
            )
...
class TaskSurveyView(SelectRelatedMixin, PrefetchRelatedMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        user_survey_list = [
            x for x in survey_list if x.user == self.request.user
        ]  # TODO: Lazy
...
class TaskCreateView(
    ...
    def case(self):
        return get_object_or_404(
            Case.objects.select_related("monitoring"), pk=self.kwargs["case"]
        )
...
class SurveyFillView(FormView):
    ...
    def object(self):
        try:
            return (
                Survey.objects.filter(task=self.task)
                .of_user(user=self.request.user, light_user=self.request.light_user)
                .all()[0]
            )
        except IndexError:
            return None
