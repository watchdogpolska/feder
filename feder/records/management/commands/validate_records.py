    ...
    def handle(self, *args, **options):
        ...
        for record in my_iter(Record.objects.all()):
            if record.content_object:
                continue
            self.stdout.write(self.style.ERROR("Invalid record (pk={})".format(record.pk)))
            if options['fix']:
                self.stderr.write(self.style.SUCCESS("Removed record (pk={})".format(record.pk)))
            if options['fix'] and not options['dry_run']:
                record.delete()

        related_models = [field.related_model for field in Record._meta.related_objects
        ...
        for related_model in my_iter(related_models):
            ...
            for obj in my_iter(related_model.objects.all(), desc=verbose_name):
                if obj.record:
                    continue
                msg = "Invalid {} (pk={}, record={})".format(verbose_name,
                                                             obj.pk,
                                                             repr(obj.record))
