...
class QuestionFormTestCase(ObjectMixin, TestCase):
    def test_construct_form(self):
        ...
        self.assertSequenceEqual(
            sorted(form.fields.keys()), sorted(["position", "genre"])
        )


class QuestionDefinitionFormTestCase(QuestionObjectMixin, TestCase):
    def test_construct_form(self):
        form = QuestionDefinitionForm(user=self.user, instance=self.question)
        html = render_form(form)
        self.assertIn("input", html)
        self.assertSequenceEqual(
    ...
    def test_save(self):
        ...
        self.question.save()
        form = QuestionDefinitionForm(
            user=self.user,
            instance=self.question,
            data={"name": "Foo bar", "help_text": "Foo"},
        )
...
class QuestionCreateViewTestCase(ObjectMixin, PermissionStatusMixin, TestCase):
    ...
    def get_url(self):
        return reverse(
            "questionaries:question_create", kwargs={"pk": self.questionary.pk}
        )
...
class QuestionMoveViewTestCase(QuestionObjectMixin, PermissionStatusMixin, TestCase):
    ...
    def get_url(self):
        return reverse("questionaries:question_down", kwargs={"pk": self.question.pk})
...
class QuestionDeleteViewTestCase(QuestionObjectMixin, PermissionStatusMixin, TestCase):
    ...
    def get_url(self):
        return reverse("questionaries:question_delete", kwargs={"pk": self.question.pk})
    ...
    def test_get_success_url(self):
        ...
        self.assertRedirects(response, self.questionary.get_absolute_url())
